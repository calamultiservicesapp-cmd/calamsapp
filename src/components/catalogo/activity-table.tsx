...
import {
  Plus, Pencil, Trash2, X, Loader2, CheckCircle2, AlertCircle, BookOpen
} from "lucide-react";
...
type SerializedActivity = Omit<Activity, "minHours"> & { minHours: string };
        ...
        <form action={formAction} className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-1.5">
              ...
              <textarea
                name="descriptionEs"
                defaultValue={activity?.descriptionEs}
                required
                rows={3}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-1.5">
              ...
              <select
                name="defaultPersonnelType"
                defaultValue={activity?.defaultPersonnelType}
                required
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  type="number"
                  name="minHours"
                  defaultValue={activity?.minHours}
                  required
                  step="0.5"
                  min="0.5"
                  className="w-full h-10 px-3 rounded-md border border-slate-300 dark:border-slate-700 bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md border border-slate-200 dark:border-slate-800 p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-heading text-slate-800 dark:text-white">Eliminar Actividad</h3>
        </div>
...
export function ActivityTable({ activities }: { activities: SerializedActivity[] }) {
  ...
  return (
    <>
      {modal === "create" && (
        <ActivityForm mode="create" onClose={() => setModal(null)} />
      )}
      {modal === "edit" && selected && (
        <ActivityForm mode="edit" activity={selected} onClose={() => { setModal(null); setSelected(null); }} />
      )}
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-slate-400">
              ...
              <p className="text-xs mt-1">Usa el botón "Nueva Actividad" para agregar la primera.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-950/50 border-b border-slate-200 dark:border-slate-800">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Actividad (ES / EN)</th>
                    ...
                    <tr key={activity.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 dark:bg-blue-950/50 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                          {personnelLabels[activity.defaultPersonnelType]}
                        </span>
                      </td>
        ...
        <p className="text-xs text-slate-400">{filtered.length} actividad(es) encontrada(s).</p>
      </div>
    </>
  );
}
