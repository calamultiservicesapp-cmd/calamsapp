...
<head>
  ...
  <title>${isEnglish ? "Rejected Proposal" : "Propuesta Rechazada"} — ${project.name}</title>
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 40px; padding-bottom: 24px; border-bottom: 3px solid #64748b; }
    .logo-img { max-height: 70px; width: auto; object-fit: contain; opacity: 0.8; filter: grayscale(100%); }
    .doc-info { text-align: right; }
    .doc-info p { font-size: 13px; color: #64748b; }
    .doc-info .ref { font-size: 11px; color: #94a3b8; margin-top: 4px; }
    h2 { font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; color: #475569; margin-bottom: 12px; margin-top: 32px; }
    .client-box { background: #f8fafc; border-left: 4px solid #94a3b8; padding: 16px 20px; border-radius: 0 8px 8px 0; }
    .client-box p { font-size: 14px; margin-bottom: 4px; }
    .client-box .name { font-size: 18px; font-weight: 700; color: #334155; margin-bottom: 8px; }
    table { width: 100%; border-collapse: collapse; margin-top: 8px; }
    th { background: #475569; color: #fff; text-align: left; padding: 10px 14px; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; }
    ...
    .total-inner { background: #475569; color: #fff; padding: 20px 28px; border-radius: 12px; text-align: right; min-width: 240px; }
    .total-inner .label { font-size: 12px; text-transform: uppercase; letter-spacing: 1px; opacity: 0.7; }
    .total-inner .amount { font-size: 32px; font-weight: 900; color: #fff; margin-top: 4px; }
    ...
    .badge { background: #f1f5f9; color: #64748b; padding: 4px 10px; border-radius: 999px; font-size: 11px; font-weight: 600; border: 1px solid #cbd5e1; }
    @media print { 
  ...
  <div class="page">
    <div class="header">
      ...
      <div class="doc-info">
        ...
        <p class="ref">Ref: CALA-REV-${revisionId.slice(-6).toUpperCase()}</p>
      </div>
    </div>
    ...
    <div class="footer">
      <div>
        ...
        <p>${isEnglish ? "Historical Record." : "Registro Histórico."}</p>
      </div>
      <span class="badge">${isEnglish ? "Rejected Proposal" : "Propuesta Rechazada"}</span>
    </div>
  </div>
</body>
</html>
